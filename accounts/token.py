...
def get_token_generator():
    """
    Returns the token generator class based on the configuration in DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG.CLASS and
    DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG.OPTIONS
    :return:
    """
    ...
    DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG = getattr(settings, 'DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG', None)

    # check if something is in the settings object, and work with it
    if DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG:
        if "CLASS" in DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG:
            class_path_name = DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG["CLASS"]
            module_name, class_name = class_path_name.rsplit('.', 1)

            mod = import_module(module_name)
            token_class = getattr(mod, class_name)

        if "OPTIONS" in DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG:
            options = DJANGO_REST_RESETPASSWORD_TOKEN_CONFIG["OPTIONS"]
